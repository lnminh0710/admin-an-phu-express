...
import { Button, Card, CardBody, Col, Container, Input, InputGroup, Row, Table } from 'reactstrap';
...
class EcommerceCart extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (!isEmpty(products) && size(products) !== size(prevProps.cartData.products)) {
      this.setState({ productList: products });
    }
  }
                            ...
                            <tr key={product.id}>
                              ...
                              <td>
                                <h5 className="font-size-14 text-truncate">
                                  <Link to={'/ecommerce-product-details/' + product.id} className="text-dark">
                                    {product.name}
                                  </Link>
                                </h5>
                                <p className="mb-0">
                              ...
                              <td>
                                <div style={{ width: '120px' }} className="product-cart-touchspin">
                                  <InputGroup className="bootstrap-touchspin bootstrap-touchspin-injected">
                                    ...
                                    <Input type="text" value={product.data_attr} name="demo_vertical" readOnly className="form-control" />
                              ...
                              <td style={{ width: '90px' }} className="text-center">
                                <Link to="#" onClick={() => this.removeCartItem(product.id)} className="action-icon text-danger">
                                  {' '}
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(EcommerceCart));
